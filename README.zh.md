...
- `/approvals [ask|auto]` —— 本对话的批准闸门：`auto` 对所有 `x-harness.approval` 工具免询问放行（每次放行都会明确记录，静默放行正是闸门要防止的）；`ask` 恢复询问，不带参数时报告当前模式。闸门弹窗中的 `A` 键可在弹窗出现时直接设置同样的模式，因此不断询问未知工具的运行可一键批准；`a` 的含义保持不变（本会话始终允许“该”工具）
