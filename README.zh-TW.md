...
- `/approvals [ask|auto]` —— 本對話的核准閘門：`auto` 對所有 `x-harness.approval` 工具免詢問放行（每次放行都會明確記錄，無聲放行正是閘門要防止的）；`ask` 恢復詢問，不帶參數時回報目前模式。閘門彈窗中的 `A` 鍵可在彈窗出現時直接設定同樣的模式，因此不斷詢問未知工具的執行可一鍵核准；`a` 的含義保持不變（本對話一律允許「該」工具）
