...
var catalogEn = map[string]string{
	...
	"help.keys":                      "keys: alt+enter/ctrl+j newline  •  ctrl+r history search  •  ctrl+t thinking level (full → brief → off)  •  ctrl+e tool cards  •  ctrl+g thinking effort  •  pgup/pgdn or wheel scroll  •  drag select & copy  •  esc stop turn (twice)  •  ctrl+c quit",
...
var catalogZh = map[string]string{
	...
	"help.keys":                      "按键：alt+enter/ctrl+j 换行 • ctrl+r 历史搜索 • ctrl+t 思考显示（完整 → 简要 → 关闭） • ctrl+e 工具卡片 • ctrl+g 思考深度 • pgup/pgdn 或滚轮滚动 • 拖拽选择并复制 • esc 停止回合（按两次） • ctrl+c 退出",
...
var catalogZhTW = map[string]string{
	...
	"help.keys":                      "按鍵：alt+enter/ctrl+j 換行 • ctrl+r 歷史搜尋 • ctrl+t 思考顯示（完整 → 簡要 → 關閉） • ctrl+e 工具卡片 • ctrl+g 思考深度 • pgup/pgdn 或滾輪捲動 • 拖曳選取並複製 • esc 停止回合（按兩次） • ctrl+c 結束",
