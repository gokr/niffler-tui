...
var catalogEn = map[string]string{
	...
	"chat.mouseOn":                   "mouse on — wheel scrolls the transcript; click a tool card to expand it (ctrl+e cycles tool cards); copy with shift+drag",
...
var catalogZh = map[string]string{
	...
	"chat.mouseOn":                   "鼠标已开启 — 滚轮滚动记录；点击工具卡片展开（ctrl+e 循环工具卡片）；shift+拖拽复制",
...
var catalogZhTW = map[string]string{
	...
	"chat.mouseOn":                   "滑鼠已開啟 — 滾輪捲動記錄；點擊工具卡片展開（ctrl+e 循環工具卡片）；shift+拖曳複製",
